<?php
     ...
     */
    class APartner extends Partner
    {
        /**
        ...
        public function rules()
        {
            ...
            return array(
                array('status, phone, cp_id', 'numerical', 'integerOnly' => TRUE),
                array('phone, email, return_url, cp_id, name', 'required'),
                array('description', 'length', 'max' => 500),
                array('email', 'unique'),
                array('email', 'email'),
                array('name', 'unique'),
                array('cp_id', 'unique'),
                array('return_url', 'unique'),
         ...
         *
         * @return APartner the static model class
         */
        ...
        {
            $this->name        = Yii::app()->input->purify($this->name);
            $this->description = Yii::app()->input->purify($this->description);
            $this->return_url  = Yii::app()->input->purify($this->return_url);
            $this->phone       = Yii::app()->input->purify($this->phone);
            $this->email       = Yii::app()->input->purify($this->email);
            $this->cp_id       = Yii::app()->input->purify($this->cp_id);
            if ($this->status == 1) {
                $this->status = 10;
            }
        ...
        public function genKeyAES($lengthChars = 32)
        {
            if ($lengthChars <= 0) {
                return FALSE;
            } else {
