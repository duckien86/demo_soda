...
//        const AESKEY_LENGTH = 32;
        const AESKEY_LENGTH = 20;
        ...
        /**
         * Check tồn tại partner.
         *
         * @return bool
         */
        public function checkExist($pId)
        {
            $criteria            = new CDbCriteria();
            $criteria->condition = 'cp_id=:cp_id and (return_url is not null OR return_url !="") and status=:status';
            $criteria->params    = array(':cp_id' => $pId, ':status' => self::ACTIVE);
            $partner             = Partner::model()->find($criteria);
            if ($partner && $partner->aes_key != '' && strlen($partner->aes_key) == self::AESKEY_LENGTH) {
                return $partner;
            }
