...
<div class="x_panel">
    ...
    <div class="row">
        <?php if ((isset($order_detail) && !empty($order_detail))
            && (isset($order_state) && !empty($order_state))
            && (isset($order_shipper) && !empty($order_shipper))
...
<script type="text/javascript">
    ...
    function loadDetail(value, order_id, type) {
        if (value != 0 && order_id != '') {
            $.ajax({
                type: "POST",
                url: '<?=Yii::app()->controller->createUrl('aTraffic/proxy')?>',
                crossDomain: true,
//                dataType: 'json',
                data: {
                    value: value,
