    ...
    <?php
        if ($package):
            ...
            foreach ($package as $item):
                ...
                <div class="col-md-6 pack_chk">
                    <input class="chk_package" id="AOrders_package_<?= $i ?>" value="<?= $item->id ?>" <?= $checked ?>
                           type="checkbox"
                           name="AOrders[package][]">
                    <label for="AOrders_package_<?= $i ?>">
                        <?php
...
<script>
    function getPackageDetail(package_id) {
        $.ajax({
            type: "POST",
            url: "<?=Yii::app()->controller->createUrl('aCheckout/getPackageDetail');?>",
            crossDomain: true,
            dataType: 'json',
