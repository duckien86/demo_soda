...
//        'autoOpen' => true,
    ...
    <div class="modal-header">
        ...
        <h4 class="modal-title">Upload File Kho thẻ doanh nghiệp</h4>
    </div>
    <div class="modal-body">
        <form method="post" id="formUploadCardStoreBusiness" enctype="multipart/form-data" onsubmit="return false">
            <input type="hidden" name="YII_CSRF_TOKEN" value="<?php echo Yii::app()->request->csrfToken?>"/>
            <div class="form-group">
                <div id="ACardStoreBusiness_upload_msg"></div>
                <div id="ACardStoreBusiness_upload_error" class="error"></div>

                <img class="loading_gif right in-block hidden" src="<?php echo Yii::app()->theme->baseUrl?>/images/loading.gif"/>

                <input class="in-block unset-w" id="ACardStoreBusiness_upload" name="ACardStoreBusiness[upload]" type="file" accept="text/plain">
            </div>
            ...
            <div id="card_upload_detail">
                <?php echo $this->renderPartial('/aCardStoreBusiness/_table_card')?>
            </div>
        </form>
    </div>
...
<script>
    $(document).ready(function () {
        ...
        var buttonOk = $('#btnOk');
        var error = $('#ACardStoreBusiness_upload_error');
        var msg = $('#ACardStoreBusiness_upload_msg');
        ...
        $('#ACardStoreBusiness_upload').on('change', function () {
            ...
            var value = $(this).val();
            if(value.length){
                var form_data = new FormData(document.getElementById("formUploadCardStoreBusiness"));//id_form
                loading.removeClass('hidden');
                $.ajax({
                    url: '<?php echo Yii::app()->controller->createUrl('aCardStoreBusiness/getFileImportContent')?>',
                    type: 'post',
                    dataType: 'json',
                    data: form_data,
        ...
        buttonSubmit.on('click', function () {
            ...
            loading.removeClass('hidden');
            $.ajax({
                url: '<?php echo Yii::app()->controller->createUrl('aCardStoreBusiness/uploadFileImport')?>',
                type: 'post',
                dataType: 'json',
                data: form_data,
                enctype: 'multipart/form-data',
                ...
                success: function (result) {
                    ...
                    if(result.error){
                        ...
                        msg.css('color', '#28a745');
                        error.html('');
                        $.fn.yiiGridView.update('acardstorebusiness-grid');
                        $('#tableCard tbody').html(emptyHtml);
                        $('#tableCardSummary thead').html(emptyHtmlSummaryHead);
                        $('#tableCardSummary tbody').html(emptyHtmlSummaryBody);
                    }
