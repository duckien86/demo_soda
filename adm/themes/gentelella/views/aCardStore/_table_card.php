...
<?php echo $this->renderPartial('/aCardStore/_table_card_summary', array('data' => $data))?>
            ...
            foreach ($data as $card){ ?>
                <tr>
                    ...
                    <td>
                        ...
                        <input name="ACardStore[data][<?php echo $row?>][serial]" type="hidden" value="<?php echo $card->serial?>" />
                    </td>
                    <td>
                        <?php echo CHtml::encode($card->pin)?>
                        <input name="ACardStore[data][<?php echo $row?>][pin]" type="hidden" value="<?php echo $card->pin?>" />
                    </td>
                    <td>
                        <?php echo CHtml::encode(number_format($card->value,0,',','.'))?>
                        <input name="ACardStore[data][<?php echo $row?>][value]" type="hidden" value="<?php echo $card->value?>" />
                    </td>
                    <td>
                        <?php echo date("d-m-Y H:i:s", strtotime($card->expired_date))?>
                        <input name="ACardStore[data][<?php echo $row?>][expired_date]" type="hidden" value="<?php echo date("d-m-Y H:i:s",strtotime($card->expired_date))?>" />
                    </td>
                </tr>
            <?php }
        }?>
