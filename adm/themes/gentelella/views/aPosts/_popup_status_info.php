            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <?php if ($status == APosts::INACTIVE) { ?>
                        <h4 class="modal-title">Lý do muốn ẩn hoặc cấm bình luận bài đăng</h4>
                    <?php } else { ?>
                        <h4 class="modal-title">Lý do bạn phải khôi phục lại bài đăng</h4>
                    <?php } ?>
                </div>
...
<script type="text/javascript">
    function ajaxUpdateResult(id, info, status, sso_id) {
        if (info != '') {
            $.ajax({
                type: "POST",
                url: '<?=Yii::app()->controller->createUrl('aPosts/updateStatusInfo')?>',
                crossDomain: true,
            ...
            alert("Bạn phải nhập lý do ẩn bài đăng!");
        }
    }
</script>
