...
<div id="order">
    ...
    <?php if($model->status == TOrders::ORDER_DRAFTS){
        $this->step = OrderController::STEP_CONFIRM_ORDER;
        $this->renderPartial('/order/_block_form_wizard');
    }?>
    ...
    <div class="row">
            ...
            <?php $this->widget('booster.widgets.TbDetailView', array(
                ...
                'attributes' => array(
                    ...
                    array(
                        ...
                        'value'       => function ($data) {
                            return Chtml::encode(date('d/m/Y', strtotime($data->create_time)));
                        },
                        'htmlOptions' => array('style' => 'vertical-align:middle;'),
                    ),
                    array(
                    ...
                    array(
                        ...
                        //'visible'   => (Yii::app()->user->user_type == TUsers::USER_TYPE_KHDN)
                    ),
                ),
            )); ?>
        </div>
        ...
        <div class="col-md-12" style="margin-top: 30px;">
            ...
            <?php
            $this->renderPartial('/order/_block_order_details', array(
...
//                        'content' => $this->renderPartial('/order/_block_order_details', array(
//                            'model_details' => $model_details,
//                            'model' => $model),
//                        TRUE),
//                        'active'  => TRUE,
...
//            ));
    ...
    <?php if($model->status >= TOrders::ORDER_ASSIGNED){
        ...
        echo CHtml::link(CHtml::encode(Yii::t('tourist/label','report')), Yii::app()->createUrl('order/report', array('id' => $model->id)), array(
            'class' => 'btn btn-lg',
            'id'    => 'btn-report',
        ));
        echo '</div>';
    ...
    <?php if($model->status == TOrders::ORDER_DRAFTS){
        echo '<div class="col-md-12 step_confirm_order" style="margin-top: 30px; margin-bottom: 30px">';
        echo CHtml::link(CHtml::encode(Yii::t('tourist/label','edit')), Yii::app()->createUrl('order/update', array('id' => $model->id)), array(
            'class' => 'btn btn-lg',
            'id'    => 'btn-update',
        ));
        echo CHtml::link(CHtml::encode(Yii::t('tourist/label','confirm')), Yii::app()->createUrl('order/confirm', array('id' => $model->id)), array(
            'class' => 'btn btn-lg',
            'id'    => 'btn-confirm',
        ));
        echo '</div>';
