<?php
    ...
    class AOrdersData extends CommonOrdersData
    {
        ...
        public function removeKeepMsisdn($data, $reason = 'unknown')
        {
            ...
            $order_id = '';
            /*---------hủy giữ số đã có order_id-----*/
            $order_data = Yii::app()->session['orders_data'];
            if($order_data && $order_data->orders && $order_data->orders->id){
                $order = AOrders::model()->findByPk($order_data->orders->id);
                if($order){
                    $type     = 'web_remove_keep_with_order_id';
                    $order_id = $order_data->orders->id;
                }
            }
            $data['order_id'] = $order_id;
            $data['user_name'] = Yii::app()->user->name;
            /*---------./END hủy giữ số đã có order_id-----*/
            $id   = Yii::app()->request->csrfToken;
