...
//            }
    ...
    public function searchRemunerationSimDetail($dataProvider = TRUE)
    {
        ...
        if(!empty($this->start_date) && !empty($this->end_date)){
            ...
            if(!$result){
                ...
                $criteria = new CDbCriteria();
                $criteria->select = "t.*,
                    (SELECT time FROM tbl_net_data nd WHERE nd.msisdn = t.msisdn AND nd.type = 'sim') AS 'active_time'
                ";
    ...
    public function searchRemunerationPackage($dataProvider = TRUE)
    {
        ...
        if(!empty($data_detail)){
            ...
            foreach ($data_detail as $item){
                if(isset($data_raw[$item->inviter_code][$item->package_code])){
                    $data_raw[$item->inviter_code][$item->package_code]['total']++;
                    $data_raw[$item->inviter_code][$item->package_code]['revenue'] += $item->price;
                    $data_raw[$item->inviter_code][$item->package_code]['rose'] += $item->amount;
                }else{
                    $data_raw[$item->inviter_code][$item->package_code]['name'] = $item->product_name;
                    $data_raw[$item->inviter_code][$item->package_code]['bundle'] = $item->bundle;
                    $data_raw[$item->inviter_code][$item->package_code]['total'] = 1;
                    $data_raw[$item->inviter_code][$item->package_code]['revenue'] = $item->price;
                    $data_raw[$item->inviter_code][$item->package_code]['rose'] = $item->amount;
                }
            }
    ...
    public function searchRemunerationPackageDetail($dataProvider = TRUE)
    {
        ...
        if(!empty($this->start_date) && !empty($this->end_date)){
            ...
            if(!$result){
                ...
                $criteria = new CDbCriteria();
                $criteria->select = "t.*,
                    (SELECT time FROM tbl_net_data nd WHERE nd.msisdn = t.msisdn AND nd.type = 'package') AS 'active_time',
    ...
    public function searchRemunerationConsumeDetail($dataProvider = true)
    {
        ...
        if(!empty($this->start_date) && !empty($this->end_date)){
            ...
            if(!$result){
                ...
                $criteria = new CDbCriteria();
                $criteria->select = "t.*,
                    (SELECT nd.time FROM tbl_net_data nd WHERE nd.msisdn = t.msisdn AND nd.type = 'sim' ORDER BY nd.time ASC LIMIT 1) AS 'active_time'
                ";
                $criteria->condition = "(t.campaign_category_id = :campaign_category_id)
                    AND (t.created_on >= :start_date)
                    AND (t.created_on <= :end_date)
