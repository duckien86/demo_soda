...
class Report extends CFormModel
{
    ...
    public function rules()
    {
        return array(
            array('start_date, end_date, ctv_id, province_code, district_code,
                ...
                delivery_type, card_type, receive_status, on_detail', 'safe')
        );
    }
    ...
    public function getRenueveIndexOverview()
    {
        ...
        $criteria->select = "SUM(od.price) as renueve,
                             ...
                             s.type as type";
        if ($this->start_date && $this->end_date) {
            $criteria->condition = "od.type IN ('sim','package','price_term') 
                                AND ot.delivered = 10
        ...
        foreach ($key_array as $key) {
            ...
            foreach ($data as $value) {
                if ($value->type == $key) {
                    ...
                    if ($value->order_type == 'sim' || $value->order_type == 'price_term') {
...
//            ));
    ...
    public function getPackageRenueve($type = '')
    {
        ...
        if ($type == '') {
            $criteria->condition = "od.type IN ('package','sim') and ot.create_date >='$this->start_date' 
...
//        }
    ...
    public function getEmailAccumulatedTourist()
    {
        ...
        $criteria->condition = "t.create_time >= '2018-03-01 00:00:00' AND t.create_time <= '$this->end_date'";
     ...
     * @return ROrderDetails[] | CArrayDataProvider
     */
    public function searchRenuevePackageSimKit($dataProvider = TRUE)
    {
        ...
        if(!empty($data_detail)){
            ...
            foreach ($data_detail as $item){
                ...
                $renueve    = $item->renueve;

                if(isset($data_raw[$item_id])){
                    $data_raw[$item_id]['total_package'] ++;
                    $data_raw[$item_id]['renueve_package'] += $renueve;
                }else{
            ...
            foreach ($data_raw as $key => $item){
                $order_detail = new ROrderDetails();
                $order_detail->item_id          = $key;
                $order_detail->item_name        = $item['item_name'];
                $order_detail->total_package    = $item['total_package'];
                $order_detail->renueve_package  = $item['renueve_package'];
                $data[] = $order_detail;
            }
        }
            ...
            return new CArrayDataProvider($data, array(
                ...
                'pagination' => array(
                    'params'   => array(
                        ...
                        'ReportForm[start_date]'        => $this->start_date,
                        'ReportForm[end_date]'          => $this->end_date,
                        "ReportForm[province_code]"     => $this->package_group,
                        "ReportForm[sale_office_code]"  => $this->sale_office_code,
                        "ReportForm[package_id]"        => $this->package_id,
    ...
    public function searchDetailRenuevePackageSimKit($dataProvider = TRUE)
    {
        ...
        $criteria->select = "t.id,
                ...
                (SELECT od1.item_name FROM tbl_order_details od1 WHERE od1.order_id = t.id AND od1.type = 'sim') AS 'sim',
            ...
            ";
        $criteria->condition = "
                (t.id IN (SELECT order_id FROM tbl_order_details WHERE type = 'sim'))
                ...
                AND (t.agency_contract_id IN (SELECT id FROM tbl_agency_contract WHERE agency_id = '".Yii::app()->user->agency."'))   
            ";
        $criteria->order = 't.create_date ASC';
    ...
    public function searchRenueveSim($dataProvider = TRUE)
    {
        ...
        if(!empty($data_detail)){
            foreach ($data_detail as $item){
                $item_type = $item->type_sim;
                $item_renueve = $item->renueve_sim;
                $item_price_term = $item->price_term;

                if(isset($data_raw[$item_type])){
                    $data_raw[$item_type]['total_sim'] ++;
                    $data_raw[$item_type]['renueve_sim'] += $item_renueve;
                    $data_raw[$item_type]['renueve_term'] += $item_price_term;
                }else{
                    $data_raw[$item_type]['total_sim'] = 1;
                    $data_raw[$item_type]['renueve_sim'] = $item_renueve;
                    $data_raw[$item_type]['renueve_term'] = $item_price_term;
                }
            }
            ...
            foreach ($data_raw as $key => $item){
                $order = new ROrders();
                $order->type_sim    = $key;
                $order->total_sim   = $item['total_sim'];
                $order->renueve_sim = $item['renueve_sim'];
                $order->renueve_term = $item['renueve_term'];
                $data[] = $order;
            }
        }
    ...
    public function searchDetailRenueveSim($dataProvider = TRUE)
    {
        ...
        $criteria->select = "
                ...
                (SELECT price FROM tbl_order_details WHERE order_id = t.id AND type = 'sim') AS 'renueve_sim'
        ";
    ...
    public function searchDetailRenuevePackageSingle($dataProvider = TRUE)
    {
        ...
        $criteria->condition = "
            ...
            AND (t.id NOT IN (SELECT order_id FROM tbl_order_details WHERE type = 'sim'))
            AND (t.id IN (SELECT order_id FROM tbl_order_state 
                WHERE create_date >= '$start_date' 
                    AND create_date <= '$end_date' 
                    AND id = (SELECT MIN(os2.id) FROM tbl_order_state os2 
    ...
    public function searchDetailRevenueSynthetic($dataProvider = TRUE)
    {
        ...
        $criteria->select = "
                ...
                t.receive_cash_date,
                t.payment_method,
                (SELECT create_date FROM tbl_order_state 
                    WHERE id = (SELECT MIN(os2.id) FROM tbl_order_state os2 
                        WHERE os2.order_id = t.id AND os2.delivered = 10
                    )
                ) AS 'delivery_date',
                CASE
