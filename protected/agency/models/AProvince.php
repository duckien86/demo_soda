<?php
    ...
    class AProvince extends Province
    {
        ...
        {
            $result = '';
            $cache_key = "AProvince_getProvinceNameByCode_$code";
            if($cache){
        ...
        /**
         * Lấy tất cả tỉnh theo agency_id
         */
        public function getAllProvince($all = false)
        {
            $criteria = new CDbCriteria();
            ...
            if(!$all && !empty(Yii::app()->user->agency)){
                $province_code = User::model()->findAllByAttributes(array('agency_id' => Yii::app()->user->agency));
                $province_code = array_filter(array_keys(CHtml::listData($province_code, 'province_code', 'id')), 'strlen');
                $criteria->addInCondition('code', $province_code);
            }
            $data = Province::model()->findAll($criteria);
