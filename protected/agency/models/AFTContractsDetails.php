<?php
    ...
    class AFTContractsDetails extends FTContractsDetails
    {
                    ...
                    if (isset($order->contract_id)) {
                        if (!empty($order->contract_id)) {
                            $contract_details = AFTContractsDetails::model()->findByAttributes(
                                array('contract_id' => $order->contract_id, 'item_id' => $item_id)
                            );
                            if ($contract_details) {
        ...
        public static function getDiscountLabel($order_id, $item_id){
            $result = '0';
            $order = AFTOrders::model()->findByAttributes(array('id' => $order_id));
            if ($order) {
                $contract_details = AFTContractsDetails::model()->findByAttributes(
                ...
                if ($contract_details) {
                    if (!empty($contract_details->price_discount_percent)) {
                        $result = number_format($contract_details->price_discount_percent, 0, ',', '.') . '%';
                    } else if(!empty($contract_details->price_discount_amount)){
                        $result = number_format($contract_details->price_discount_amount, 0, ',', '.') . 'VND';
                    }
                }
            }
            return $result;
        ...
        public function validateContractDetails($details)
        {
            if ($details) {
                foreach ($details as $key => $item) {
                    if($item['type'] == AFTContractsDetails::TYPE_PERCENT){
...
//                        if($package->type == AFTPackage::FT_PACKAGE_TYPE_KHDN){
//                            $price_discount = 0; //không chiết khấu cho gói của KHDN
//                        }
