...
//            OtpForm::unsetSessionHtmlOrder();
            if (AOrders::checkOrdersSessionExists() === FALSE) {
                ...
                $check_password = true;
                if(Yii::app()->params->confirm_password && !Yii::app()->session['order_confirm_password']){
                ...
                if ($modelSim->type == ASim::TYPE_PREPAID) { //check sim type=>get package
                    ...
                    if (!empty($orders_data->package_sim_kit)) {//check package(SimkitController)
                        ...
                        if (!empty($GLOBALS['config_common']['package']['selected'])
                            ...
                            $package = APackage::getPackageByAgency(false, AAgencyPackage::DISPLAY_IN_BUY_PACKAGE);
                            //array slide package
                            /*if (!empty(Yii::app()->params->checkout_prepaid)) {
                                $package = APackage::getListPackageById(Yii::app()->params->checkout_prepaid);
                            } else {
                ...
                if (isset($_POST['ASim']) && isset($_POST['AOrders'])) {
                    ...
                    if(empty($modelOrder->address_detail)){
                        $modelOrder->address_detail = !empty(Yii::app()->user->brand_offices_id) ? Yii::app()->user->brand_offices_id :  Yii::app()->user->agency;
                    }
                    // validate ajax modal
                    if (isset($_POST['ajax']) && $_POST['ajax'] === 'true') {
                        ...
                        /*if (empty(Yii::app()->params->checkout_prepaid)
                            ...
                            echo CJSON::encode(array('code' => 1, 'msg' => 'success validation'));
                            Yii::app()->end();
                        /*}else{
                            $modelOrder->addError('package', Yii::t('web/portal', 'err_required_package'));
                            echo CJSON::encode($modelOrder->errors);
                            Yii::app()->end();
                        }*/
                    }
                    //.END validate ajax modal
                    if ($modelSim->validate() && $modelOrder->validate() && $check_password) {
                        ...
                        /*if (empty(Yii::app()->params->checkout_prepaid)
                            ...
                            if ($orders_data->buySim($data)) {
                                ...
                                $agency_order = new AAgencyOrder();
                                $agency_order->order_id = $orders_data->orders->id;
                                $agency_order->agency_id = Yii::app()->user->agency;
                                $agency_order->agency_contract_id = AAgencyContract::getCurrentAgencyContractActive();
                                $agency_order->save();
...
//            $data = str_replace(array('-', '_'), array('+', '/'), $string);
        ...
        public function actionConfirmPassword(){
            ...
            $user = new UserIdentity(Yii::app()->user->name, Yii::app()->request->getParam('conform_password'));
            ...
            if($check){
                ...
                Yii::app()->session['order_confirm_password'] = true;
            }else{
                $result['msg'] = 'Mật khẩu sai';
            }
