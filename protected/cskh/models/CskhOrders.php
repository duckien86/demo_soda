...
class CskhOrders extends Orders
{
    ...
    public function rules()
    {
        ...
        return array(
            ...
            array('sso_id, promo_code, invitation, full_name, address_detail, otp, brand_offices, sale_office_code', 'length', 'max' => 255),
            array('phone_contact', 'length', 'max' => 20),
            array('customer_note', 'length', 'max' => 500),
            array('last_update,id_dhsx', 'safe'),
            // The following rule is used by search().
            // @todo Please remove those attributes that should not be searched.
            array('id, sso_id, promo_code, invitation, create_date, sale_office_code, last_update, shipper_id,sim, delivery_type, payment_method, full_name, district_code, province_code, address_detail, phone_contact, customer_note, otp,id_dhsx', 'safe', 'on' => 'search'),
        );
    }
    ...
    public function search($post = FALSE)
    {
        ...
        if ($this->status_shipper != '') {
            if ($this->status_shipper != '3') { // Không gửi trả.
        ...
        if (!ADMIN && !SUPER_ADMIN && !$post) {
            return new CActiveDataProvider($this, array(
                ...
                'pagination' => array(
                    'params' => array(
                        "CskhOrders[sale_office_code]" => isset(Yii::app()->user->sale_offices_id) ? Yii::app()->user->sale_offices_id : '',
                        "CskhOrders[province_code]" => isset(Yii::app()->user->province_code) ? Yii::app()->user->province_code : '',
                        "CskhOrders[brand_offices_id]" => isset(Yii::app()->user->brand_offices_id) ? Yii::app()->user->brand_offices_id : '',
                        "CskhOrders[start_date]" => $this->start_date,
                        "CskhOrders[end_date]" => $this->end_date,
                        "CskhOrders[phone_contact]" => $this->phone_contact,
                        "CskhOrders[sim]" => $this->sim,
                    ),
                    'pageSize' => 30,
                ),
            ));
            ...
            return new CActiveDataProvider($this, array(
                ...
                'pagination' => array(
                    'params' => array(
                        "CskhOrders[sale_office_code]" => $this->sale_office_code,
                        "CskhOrders[province_code]" => $this->province_code,
                        "CskhOrders[brand_offices_id]" => $this->brand_offices_id,
                        "CskhOrders[start_date]" => $this->start_date,
                        "CskhOrders[end_date]" => $this->end_date,
                        "CskhOrders[phone_contact]" => $this->phone_contact,
                        "CskhOrders[sim]" => $this->sim,
    ...
    public function search_detail($start_date = '', $end_date = '', $shipper_id = '')
    {
        ...
        if ($shipper_id != '') {
            return new CActiveDataProvider('CskhOrders', array(
                'criteria' => $criteria,
                'pagination' => array(
                    'params' => array(
                        "id" => $shipper_id,
    ...
    public function search_complete($post = FALSE)
    {
        ...
        $criteria->addCondition("(os.confirm =10 or os.paid=10) and t.delivery_type =2 and od.type='sim'");
    ...
    public static function getStatus($order_id)
    {
        ...
        $state = CskhOrderState::model()->findAll($criteria);


        if ($state) {
            $state = CskhOrderState::model()->findAll($criteria)[count($state) - 1];
            if ($state->confirm == 10) {
    ...
    {
        if ($order_id) {
            $sim = Sim::model()->findByAttributes(array('order_id' => $order_id));
            if (isset($sim)) {
                if ($sim->type == 1) {
                    return "Trả trước";
                } else if ($sim->type == 2) {
    ...
    public function getTrafficStatus($order_id)
    {
        ...
        if ($order_id) {
            ...
            if ($shipper_order) {
                ...
                } else if ($shipper_order->order_status == 2) {
    ...
    public function getListOrderBySim($sim)
    {
        ...
        $criteria->join = "INNER JOIN {{order_details}} od ON od.order_id =t.id";
        $data = CskhOrders::model()->findAll($criteria);
    ...
    public static function getShipperDetail($id)
    {
        ...
        if ($shipper) {
            ...
            return new CActiveDataProvider("CskhShipper", array(
                'criteria' => $criteria,
                'pagination' => array(
                    'pageSize' => 10,
                ),
    ...
    {
        if ($order_id) {
            $sim = CskhOrderDetails::model()->findByAttributes(array('order_id' => $order_id, 'type' => 'sim'));
            if ($sim) {
    ...
    public function getTotalRenueveOrder($order_id)
    {
        if ($order_id) {
            ...
            $order_detail = CskhOrderDetails::model()->findAll($criteria)[0];
    ...
    public function registerPackage($data)
    {
        ...
        $type = 'web_register_package_cskh';
        $id = Yii::app()->request->csrfToken;
    ...
    public function cancelPackage($data)
    {
        ...
        //call api
        $response = Utils::cUrlPostJson(Yii::app()->params['socket_api_url'], $str_json, FALSE, 45, $http_code);
    ...
    public function searchfiber($dataProvider = TRUE)
    {
        ...
        $criteria->select = "t.*,
                             ...
                             (SELECT period FROM tbl_package WHERE code = (SELECT item_id FROM tbl_order_details od1 WHERE order_id = t.id AND od1.type='package_fiber'))  AS period,
                             (SELECT item_name FROM tbl_order_details WHERE order_id = t.id AND type ='package_fiber') as package_fiber_name,
        ...
        $criteria->addCondition("t.product_type = 'fiber' ");
        if ($this->id) {
        ...
        if ($this->id_dhsx) {
            $criteria->addCondition("(SELECT freedoo_order_id FROM tbl_reg_fiber rf WHERE rf.fiber_order_id = '$this->id_dhsx') = t.id");
        }
        if ($dataProvider) {
            return new CActiveDataProvider($this, array(
                'criteria' => $criteria,
                'pagination' => array(
                    'params' => array(
                        "CskhOrders[start_date]" => $this->start_date,
                        "CskhOrders[end_date]" => $this->end_date,
                        "CskhOrders[id]" => $this->id,
                        "CskhOrders[full_name]" => $this->full_name,
                        "CskhOrders[phone_contact]" => $this->phone_contact,
                        "CskhOrders[province_code]" => $this->province_code,
                    ),
                    'pageSize' => 30,
