...
//                array('start_date, end_date', 'required', 'on' => 'admin_assign'),
                ...
                array('last_update,sale_office_code, receive_cash_by, receive_cash_date', 'safe'),
                array(
                    'end_date',
                    'compare',
        ...
        {
            if (Yii::app()->controller->action->id == 'admin' || Yii::app()->controller->action->id == 'orderReceive') {
                //Lấy trạng thái giao hàng của shipper.
                if (!$this->status_shipper) {
                    $shipper_order = AShipperOrder::model()->findByAttributes(array('order_id' => $this->id));
                    if ($shipper_order) {
                        if ($shipper_order->shipper_id != '') {
                            $this->status_shipper = $shipper_order->status;
                        } else {
                            $this->status_shipper = -1;
                        }
                    } else {
                        $this->status_shipper = -1;
                    }
                    $orders = AOrders::model()->findByAttributes(array('id' => $this->id));
                    if ($orders) {
                        if (!empty($orders->receive_cash_by) && !empty($orders->receive_cash_date)) {
                            $this->status_shipper = 2;
                        } else {
                            $this->status_shipper = 1;
                        }
                    }
        ...
        /**
         * @param string $status_traffic
         * @param bool   $pagination
         *
         * @return CActiveDataProvider
         */
        public function search($status_traffic = '', $pagination = TRUE, $excel = FALSE)
        {
            ...
            $criteria->select = "t.*, os.create_date as delivered_date";
            if ($this->id != '') {
            ...
            if (SUPER_ADMIN || ADMIN) {
                ...
                if (Yii::app()->user->province_code && !(Yii::app()->user->sale_offices_id)) {
                ...
                } else if (isset(Yii::app()->user->sale_office_id) && Yii::app()->user->sale_offices_id != '' && !isset(Yii::app()->user->brand_offices_id)) {
                    ...
                    $criteria->compare('t.address_detail', Yii::app()->user->brand_offices_id);
                }
            }
            ...
            if ($this->start_date != '' && $this->end_date != '') {
                $criteria->addCondition("os.create_date >='$this->start_date' and os.create_date <='$this->end_date'");
            }
            //lọc filter status.
            if ($this->status_shipper != '' || $status_traffic != '') {
                $criteria->join = "INNER JOIN tbl_order_state os ON os.order_id = t.id
                                   INNER JOIN {{order_details}} od ON od.order_id = t.id
                                    INNER JOIN {{sim}} s ON s.order_id = t.id
                                   LEFT JOIN {{shipper_order}} so ON so.order_id = t.id";
                if ($this->status_shipper != '') {
                    if ($this->status_shipper == 1) {
                        $criteria->addCondition("so.status='" . $status_traffic . "' OR t.receive_cash_by is null OR t.receive_cash_date is null ");
                    } else {
                        $criteria->addCondition("t.receive_cash_by !='' and t.receive_cash_date !=''");
                    }
                } else {
                    if ($status_traffic == 1) {
                        $criteria->addCondition("so.status='" . $status_traffic . "' OR t.receive_cash_by is null OR t.receive_cash_date is null ");
            ...
            $criteria->addCondition("os.delivered =10  and (so.status != '" . self::NOT_SHIP . "' OR so.status is null) ");
            $criteria->group = 't.id';
