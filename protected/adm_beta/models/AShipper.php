<?php
     ...
     */
    class AShipper extends Shipper
    {
        ...
        public function rules()
        {
            ...
            return array(
                ...
                array('province_code, sale_offices_code', 'required'),
                array('status, username, full_name, address_detail,email, personal_id, phone_1, phone_2', 'required'),
                ...
                array('username', 'match', 'pattern' => '/^([a-z0-9_\.-])+$/', 'message' => "Tên đăng nhập chỉ gồm các ký tự a->z, 0->9, _ ,- và dấu chấm"),
                array('password', 'match', 'pattern' => '/^([a-z0-9_\.-])+$/', 'message' => "Mật khẩu chỉ gồm các ký tự a->z, 0->9, _ ,- và dấu chấm"),
                array('phone_1', 'authenticateMsisdn'),
//                array('phone_1', 'unique', 'className' => 'AShipper', 'attributeName' => 'phone_1', 'message' => 'Số điện thoại này đã được đăng ký!'),

                array('phone_2', 'authenticateMsisdn'),
                array('phone_2', 'unique', 'className' => 'AShipper', 'attributeName' => 'phone_2', 'message' => 'Số điện thoại này đã được đăng ký!'),
                ...
                array('id, username, password, full_name,sale_offices_code, avatar, created_date, created_by, address_detail, district_code, province_code, ward_code, brand_office_id, otp, gender, birthday, personal_id, personal_id_create_date, personal_id_create_place, status', 'safe', 'on' => 'search'),
            );
        }
        ...
        public function attributeLabels()
        {
            return array(
                ...
                'renueve_shipper'          => 'Doanh thu giao hàng',
                'rose_shipper'             => 'Hoa hồng',
...
//            $criteria->select = "t.*, sum(od.price * od.quantity) as renueve_order, count(distinct od.order_id) as total_order";
            ...
            if (!ADMIN && !SUPER_ADMIN) {
                ...
                } else if (Yii::app()->user->province_code && isset(Yii::app()->user->sale_offices_id) && !isset(Yii::app()->user->brand_offices_id)) {
                    $criteria->compare('t.sale_offices_code', Yii::app()->user->sale_offices_id);
                } else if (isset(Yii::app()->user->brand_offices_id) && Yii::app()->user->brand_offices_id != '') {
                ...
                if ($post) {
                    ...
                    if ($this->sale_offices_code != '') {
                        $criteria->compare('t.sale_offices_code', $this->sale_offices_code);
                    }
        ...
        public function getTotalOrder($id, $start_date = '', $end_date = '')
        {
            ...
            $model          = AShipperOrder::model()->count($criteria);
        ...
        public function getTotalOrderInfoByInput()
        {
            ...
            if ($this->start_date && $this->end_date) {
                ...
                if ($this->sale_offices_code != '') {
                    ...
                    $criteria->addCondition("o.sale_office_code = '" . $this->sale_offices_code . "'");
                }
                if ($this->id) {
            ...
            $data           = AShipperOrder::model()->findAll($criteria)[0];
...
//            CVarDumper::dump($criteria,10,true);die();
        ...
        public function getTotalShipperByInput()
        {
            ...
            $data           = AShipperOrder::model()->count($criteria);
        ...
        {
            $shipper = AShipper::model()->findByAttributes(array('username' => $username));
