                    ...
                    $data = new CArrayDataProvider($data, array(
                        ...
                        'pagination' => array(
                            'params'   => array(
                                ...
                                "ReportForm[receive_status]"   => isset($model->receive_status) ? $model->receive_status : '',
                            ),
                            'pageSize' => 10,
                        ),
...
//                $form->ward_code        = $model->ward_code = $_POST['ReportForm']['ward_code'];
                ...
                if ($form_validate->validate()) {
                    ...
                    $data = $model->getSimRenueve();

                    $data_detail = $model->detailRenueveSim();
                    $data_term   = $model->getSimTerm();

                    foreach ($data as $key => $value) {
                        if ($value->type_sim == ASim::TYPE_PREPAID) {
                            $data[$key]->renueve_term = 0;
                        } else {
                            $data[$key]->renueve_term = $data_term;
                        }
                    }
        ...
        public function actionPackage()
        {
            ...
            if (isset($_POST['ReportForm']) || isset($_REQUEST['ReportForm'])) {
                ...
                if ($_POST['ReportForm']['package_group'] != '') {
                    $model->package_group = $form->package_group = $_POST['ReportForm']['package_group'];
                }
...
//                $form->district_code    = $model->district_code = $_POST['ReportForm']['district_code'];
                ...
                if ($form_validate->validate()) {
                    $data        = $model->getPackageRenueve();
                    $data_detail = $model->detailRenuevePackage();
                } else {
                    $form_validate->getErrors();
                }
            }
...
//                    $new_roaming = $model->getSubscribers(ReportOci::NEW_ROAMING, ReportOci::SPS_PRODUCT_POST); // thuê bao mới
        ...
        public function actionPackageSimKit()
        {
            ...
            if (isset($_POST['ReportForm']) || isset($_REQUEST['ReportForm'])) {
                ...
                if ($form_validate->validate()) {
                    $data        = $model->getPackageRenueve(ReportForm::SIMKIT);
                    $data_detail = $model->detailRenuevePackage(ReportForm::SIMKIT);
            ...
            return $this->render('package_simkit', array('form' => $form, 'data' => $data, 'data_detail' => $data_detail, 'form_validate' => $form_validate));
        }
