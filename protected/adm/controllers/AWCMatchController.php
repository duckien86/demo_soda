...
class AWCMatchController extends AController
{
	/**
		...
		if(isset($_POST['AWCMatch']))
		{
			...
			if(!empty($model->start_time)){
				...
				if(empty($model->minute)){
					...
					if(intval($model->minute) < 10){
						$model->minute = '0'.$model->minute;
					}
				}
				$model->start_time = date('Y-m-d', strtotime(str_replace('/','-',$model->start_time)));
	...
	{
		$model=new AWCMatch('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_REQUEST['AWCMatch']))
			$model->attributes=$_REQUEST['AWCMatch'];
