            ...
            if (isset($_POST['ReportForm']) || isset($_REQUEST['ReportForm'])) {
                ...
                if ($form_validate->validate()) {
                    ...
                    if(!empty($model->msisdn)){
                        $arr_01290 = array('082', '8482', '01290', '841290');
                        if(Utils::checkPrefix($arr_01290, ($model->msisdn))) {
                            $phone = $model->msisdn;
                            $data = array();
                            $data_arr = array();
                            foreach ($arr_01290 as $prefix){
                                $model->msisdn = Utils::changePrefix($prefix, $model->msisdn);
                                $data_arr[] = $model->getMtLog();
                            }
