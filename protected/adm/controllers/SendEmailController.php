...
//        }
        ...
        public function actionSales()
        {
            ...
            set_time_limit(-1);
            $model = new Report();
            $model->end_date    = date("Y-m-d", strtotime(date("Y-m-d") . ' -1 day')) . ' 23:59:59';
            $model->start_date  = date('Y-m-d', strtotime($model->end_date . ' -3 day')) . ' 00:00:00';
...
//                'nttnhan@vnpt.vn',
//                'phuongnguyen@vnpt.vn',
//                'thu.nn@centech.com.vn',
//                'hanhdt@vnpt.vn',
//                'minhlt@vnpt.vn',
//                'hung.nm@centech.com.vn',
            'hungnm.dev94@gmail.com'
            );
            if (isset($_GET['email'])) {
                if (!empty($_GET['email'])) {
            ...
            $data_sim = $model->getDataTotalSim();
            $data_sim_tourits = $model->getDataTotalSimTourist();
            $data_merg = array_merge($data_sim,$data_sim_tourits);
            $data_result = array();
            $year = array();
            foreach ($data_merg as $key => $value) {


                if (!in_array($value['year'], $year)) {
                    array_push($year, $value['year']);
                }

            }
            foreach ($year as $key_p => $year_){
                $result_key = array(
                    'year' => '',
                    'total_sim'=>0,
                    'renueve_sim'=>0,
                    'total_sim_tourist'=>0,
                    'renueve_sim_tourist'=>0
                );

                foreach ($data_merg as $key => $value){
                    if($year_ == $value['year']){
                        $result_key['year'] = $value['year'];
                        $result_key['total_sim'] += $value['total_sim'];
                        $result_key['renueve_sim'] += $value['renueve_sim'];
                        $result_key['total_sim_tourist'] += $value['total_sim_tourist'];
                        $result_key['renueve_sim_tourist'] += $value['renueve_sim_tourist'];

                    }

                }
                $data_result[] = $result_key;
            }

            $data_renueve = $model->getRevenueDailyEmail();

            $data_accumulated = $model->getRevenueAccumulated();

            $data_order = $model->getOrderStatisticDailyEmail();
