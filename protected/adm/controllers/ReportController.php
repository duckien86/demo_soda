...
//            $this->pageTitle     = 'Query Builder';
        ...
        public function accessRules()
        {
            return array(
                array('allow',  // allow all users to perform 'index' and 'view' actions
                    'actions' => array('incentivesAgency','index', 'sim', 'package', 'card', 'getDistrictByProvice', 'getWardByDistrict', 'getBrandOfficeByDistrict','reportfiber'),
                    'users'   => array('@'),
                ),
        ...
        public function actionReportfiber(){
            ...
            if (isset($_POST['ReportForm']) || isset($_REQUEST['ReportForm'])) {
                ...
                if ($form_validate->validate()) {
                    $data = $model->getFiberOrder();
                    $data = new CArrayDataProvider($data, array(
                        'keyField' => false,
                        'pagination' => array(
                            'params'   => array(
                                'ReportForm[start_date]'            => $form->start_date,
                                'ReportForm[end_date]'              => $form->end_date,
                                "ReportForm[province_code]"         => $form->province_code,
                                "ReportForm[freedoo_order_id]"      => $form->freedoo_order_id,
                            ),
                            'pageSize' => 30,
                        ),
                    ));
            ...
            return $this->render('report_fiber', array(
                'form' => $form,
        ...
        /*
         * Báo cáo hoa hồng PT TB TS ĐLTC
         */
        public function actionIncentivesAgency(){
            $form           = new ReportForm();
            $form_validate  = new ReportForm();
            ...
            if (isset($_POST['ReportForm']) || isset($_REQUEST['ReportForm'])) {
                ...
                if ($form_validate->validate()) {
                    $data = $model->getIncentivesAgencyDetail();
                    $data = new CArrayDataProvider($data, array(
                        'keyField' => false,
                        'pagination' => array(
                            'params'   => array(
            ...
            return $this->render('incentives_agency', array(
                'form'          => $form,
                'form_validate' => $form_validate,
                'model'         => $model,
                'data'          => $data
...
//                $form->district_code    = $model->district_code = $_POST['ReportForm']['district_code'];
         ...
         */
        public function actionStatisticsSim(){
            $form           = new ReportForm();
            $form_validate  = new ReportForm();
            $model          = new Report(FALSE);
            $data           = array();
...
//                $form->district_code    = $model->district_code = $_POST['ReportForm']['district_code'];
                ...
                $form->receive_status   = $model->receive_status = $_POST['ReportForm']['receive_status'];
                $form->on_detail        = $model->on_detail = $_POST['ReportForm']['on_detail'];
                $form->channel_code     = $model->channel_code = $_POST['ReportForm']['channel_code'];
                $form->online_status     = $model->online_status = $_POST['ReportForm']['online_status'];
                $form->item_sim_type     = $model->item_sim_type = $_POST['ReportForm']['item_sim_type'];
                ...
                if ($form_validate->validate()) {
                    $data = $model->searchStatisticSim(TRUE);
                    if ($form->on_detail == 'on') {
                        $data_detail = $model->searchDetailStatisticSim(TRUE);
                    }
                }
            }

            return $this->render('statistics_sim', array(
                'form'          => $form,
                'form_validate' => $form_validate,
                'model'         => $model,
                'data'          => $data,
        ...
        /**
         * Thống kê gói cước đơn lẻ
         */
        public function actionStatisticsPackage()
        {
            $form          = new ReportForm();
            $form_validate = new ReportForm();
            $model         = new Report(FALSE);
            ...
            if (isset($_POST['ReportForm']) || isset($_REQUEST['ReportForm'])) {
                ...
                $form->online_status    = $model->online_status     = isset($_POST['ReportForm']['online_status'])? $_POST['ReportForm']['online_status'] : '';
                ...
                if ($form_validate->validate()) {
                    $data        = $model->searchStatisticPackage(TRUE);
                    if ($form->on_detail == 'on') {
                        $data_detail = $model->searchDetailStatisticPackage(TRUE);
                    }
                }
            }

            return $this->render('statistics_package', array('form' => $form, 'data' => $data, 'data_detail' => $data_detail, 'form_validate' => $form_validate));
...
//                    CVarDumper::dump($data_overview,10,true);die();
        ...
        public function actionPackageSimKit()
        {
            ...
            $form_validate = new ReportForm();
            $model         = new Report(FALSE);
            $model->sim_type = ASim::TYPE_PREPAID;
            $data          = $data_detail = array();
            if (isset($_POST['ReportForm']) || isset($_REQUEST['ReportForm'])) {
                    ...
                    $data = new CArrayDataProvider($data, array(
                        ...
                        'pagination' => array(
                            'params'   => array(
                                ...
                                "ReportForm[item_sim_type]"    => isset($model->item_sim_type) ? $model->item_sim_type : '',
                            ),
                            'pageSize' => 30,
                        ),
        ...
        public function actionActiveSim(){
            $form = new ReportForm();
            $model = new ReportOci();
            if($_REQUEST && $_REQUEST['ReportForm']){
                $form->attributes = $_REQUEST['ReportForm'];
                if($form->validate()){
            ...
            return $this->render('activeSim', array('form' => $form, 'data' => $data));
