                        ...
                        if ($sim->type == ASim::TYPE_POSTPAID) {
                            if ($sim->status == ASim::SIM_ACTIVE) { // Nếu đã khai báo rồi.Chuyển sang form kiểm tra hòa mạng.
                                ...
                                if(!empty($sim->esim_qrcode)){
                                    ...
                                    if($order_detail_package){
                                        $package = APackage::model()->findByAttributes(array('code' => $order_detail_package->item_id));
                                    }
                                    if(!empty($order->shipper_id)){
                                        $shipper = AShipper::model()->findByPk($order->shipper_id);
                                    }else{
                                        $log_sim = ALogsSim::model()->findByAttributes(array('order_id' => $order_id));
                                        if($log_sim){
                                            $user = User::model()->findByPk($log_sim->user_id);
                                        }
                                    }
                                    $return = $this->renderPartial('_popup_check_roaming_esim',array(
                                        'order_id' => $order_id,
                                        'order'     => $order,
                                        'sim'       => $sim,
                                        'package'   => $package,
                                    ...
                                    $return = $this->renderPartial('_popup_check_roaming', array('order_id' => $order_id));
                                }
                            ...
                            if ($sim->status != ASim::SIM_ACTIVE) {
                                ...
                                if(AOrderDetails::checkHeyZ($order_id)){ // kiểm tra có phải gói heyz
                                    $return = $this->renderPartial('_popup_check_exist_register_info', 
                                        array('order_id' => $order_id, 'msisdn' => $sim->msisdn));
                                }else{
                                    $return = TRUE;
                                }
                            }
...
//            TEST heyz
//            $order = AOrderDetails::model()->findByAttributes(array('order_id' => $order_id));
//            CVarDumper::dump(1, 10, true); die;
            if ($order_id && $otp) {
        ...
        public function actionResendOtp()
        {
            ...
            if ($order_id) {
                ...
                $order_details = AOrderDetails::model()->findByAttributes(array('order_id' => $order_id, 'type' => array('sim','esim')));
        ...
        public function actionCallApiRegisterSim()
        {
            ...
            $order_id       = Yii::app()->request->getParam('order_id', FALSE);
//            TEST heyZ
//            $data = $this->renderPartial('_popup_check_exist_register_info',
//                array('order_id' => $order_id,  'msisdn' => $phone_customer)
//            );
//            echo $data;
//            exit();
            ...
            if ($serial_number && $phone_customer && $order_id) {
                ...
                }else if(!isset(Yii::app()->user->token)){
                    $result['success'] = FALSE;
                    $result['msg']     = 'Token của bạn đã hết hiệu lực, vui lòng đăng nhập lại!';
                }else{
        ...
        public function actionShowRoaming()
        {
            ...
            if(!empty($sim->esim_qrcode)){
                ...
                $data = $this->renderPartial('_popup_check_roaming_esim',array(
                    'order_id' => $order_id,
                    'order'     => $order,
                    'sim'       => $sim,
                    'package'   => $package,
                ...
                $data = $this->renderPartial('_popup_check_roaming',array('order_id' => $order_id));
            }
            echo($data);
            exit();
        }
        ...
        public function actionRegisterInfo($order_id = '', $serial_number = '')
        {
            ...
            if (!isset($_REQUEST['ACustomerForm']) || isset($_POST['ACustomerForm'])) {
                ...
                if (!empty($data_response)) {
                    ...
                    $order_detail         = AOrderDetails::model()->findByAttributes(array('order_id' => $order_id, 'type' => array('sim', 'esim')));
                    $order_detail_package = AOrderDetails::model()->findByAttributes(array('order_id' => $order_id, 'type' => 'package'));
                    ...
                    if ($order_detail->item_id) {
                        $sim                                           = ASim::model()->findByAttributes(array('msisdn' => $order_detail->item_name, 'order_id' => $order_id));
            ...
            if (isset($_POST['AUploadForm'])) {
                ...
                if ($upload_form->photo_face_url != '' && $upload_form->photo_personal1_url && $upload_form->photo_personal2_url && $upload_form->photo_order_board_url) {
                    ...
                    if ($response != 1) {
                        ...
                        $tab = '_form_upload';
                        Yii::app()->cache->delete('registerSimCell_'.$order_id);
                    }
                } else {
...
//            $data = str_replace(array('-', '_'), array('+', '/'), $string);
        ...
        /**
         * kiểm tra đăng ký thông tin thuê bao
         */
        public function actionCheckExistRegisterInfo()
        {
            $order_id = Yii::app()->getRequest()->getParam("order_id", FALSE);
            $msisdn  = Yii::app()->getRequest()->getParam("msisdn", FALSE);
            $result = array(
                'success'  => FALSE,
                'msg'      => '',
            );
            if ($order_id && $msisdn) {
                $model           = new ASim();
                $model->msisdn = $msisdn;
                $data_output = $model->checkMsisdn($model->msisdn, 'backend_check_dktt', '', false);
                if(is_string ($data_output)){
                    $data_output = CJSON::decode($data_output);
                }
//                TEST heyz
//                $data_output = array('code' => 1);
                /*
                 * TODO: gọi api check tồn tại đăng ký thông tin thuê bao
                 * */
                if($data_output && $data_output['status'] && $data_output['status']['code'] == 1){
                    $result = array(
                        'success'  => TRUE,
                        'msg'      => 'Hoàn thành giao hàng!',
                    );
                    $order_state = new AOrderState();
                    $order_state->order_id = $order_id;
                    $order_state->confirm = AOrderState::UNCONFIRMED;
                    $order_state->paid = AOrderState::UNPAID;
                    $order_state->delivered = AOrderState::DELIVERED;
                    $order_state->create_date = date("Y-m-d H:i:s");
                    $order_state->save();
                }else{
                    $result = array(
                        'success'  => FALSE,
                        'msg'      => 'Thuê bao chưa đc đăng ký thông tin!',
                    );
                }
            }else{
                $result['msg'] = 'Thiếu thông tin thuê bao';
            }
            $data = $this->renderPartial('_popup_check_exist_register_info',
                array('result' => $result, 'order_id' => $order_id, 'msisdn' => $msisdn)
            );
            echo $data;
            exit();
        }
