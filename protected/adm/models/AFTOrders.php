...
class AFTOrders extends FTOrders
{
    ...
    public $total_sim_tourist;
    public $renueve_sim_tourist;
    public $year;
    ...
    public function search()
    {
        ...
        if($this->customer){
            $criteria->compare('u.id', $this->customer, FALSE);
        }
...
//            }
    ...
    public static function getProvinceCodeById($id)
    {
        $cache_key = "AFTOrders_getProvinceCodeById_$id";
        $result = Yii::app()->cache->get($cache_key);
        if($result){
            return $result;
        }
