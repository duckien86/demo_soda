...
//                array('start_date, end_date', 'required', 'on' => 'admin_assign'),
                ...
                array('last_update,sale_office_code, receive_cash_by, receive_cash_date, delivered_date,
                    ...
                    receiver, type_sim, item_sim_type', 'safe'),
                array(
                    'end_date',
                    'compare',
        ...
        public function search($dataProvider = TRUE)
        {
            ...
            $criteria->select = "
                ...
                (SELECT price FROM tbl_order_details WHERE order_id = t.id AND type IN ('sim','esim')) AS 'price_sim',
                ...
                CASE
                    ...
                    THEN (SELECT full_name FROM tbl_shipper sp WHERE id = t.shipper_id AND sp.status = 1 )
                    ELSE ''
                END AS 'shipper_name',
                CASE
                ...
                return new CActiveDataProvider($this, array(
                    ...
                    'pagination' => array(
                        'params'   => array(
                            ...
                            "ATraffic[item_sim_type]"    => $this->item_sim_type,
                            "ATraffic[id]"               => $this->id,
                        ),
                        'pageSize' => 30,
                    ),
...
//
        ...
        {
            if(in_array($type, array('sim', 'esim'))){
                $type = array('sim', 'esim');
            }
            ...
            if ($type != '') {
                ...
                if ($all) {
                    if (isset($state) && $state->type == ASim::TYPE_POSTPAID) {
                        $criteria->condition = "order_id ='" . $order_id . "' and type IN('sim','esim','price_term')";
                    } else {
                        $criteria->condition = "order_id ='" . $order_id . "' and type IN('sim','esim','package','price_term')";
                    }
                } else {
                    $criteria->condition = "order_id ='" . $order_id . "' and type IN('sim','esim','package')";
                }
