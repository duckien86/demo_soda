<?php
    ...
    class AReportAT
    {
        ...
        public function getSimOverView()
        {
            ...
            if ($this->start_date && $this->end_date) {
                $criteria->condition = "create_date >= '$this->start_date' and create_date <='$this->end_date' AND t.amount > 0";
            }
        ...
        public function getSimDetails($excel = FALSE)
        {
            ...
            $criteria->select = "t.*,
                    (SELECT type FROM tbl_order_details od WHERE od.order_id = t.order_id AND od.type IN ('sim','esim')) AS 'item_sim_type'
                ";

            if ($this->start_date && $this->end_date) {
                $criteria->condition = "order_create_date >= '$this->start_date' and order_create_date <='$this->end_date' AND t.amount > 0";
            }
            if ($this->province_code != '') {
                $criteria->addCondition("order_province_code ='" . $this->province_code . "'");
            }
            ...
            $data = new CActiveDataProvider('ACommisionsSimDetails', array(
                'criteria'   => $criteria,
                'sort'       => array('defaultOrder' => 't.order_create_date asc'),
                'pagination' => array(
                    'params'   => array(
                        'get'                          => 1,
        ...
        public function getPackageOverView()
        {
            ...
            if ($this->package_id != '') {
                $criteria->addCondition("item_id ='" . $this->package_id . "'");
            }
            if ($this->status != '') {
                $criteria->addCondition("status ='" . $this->status . "'");
            }
        ...
        public function getAffiliateOverView()
        {
            ...
            if(!$data){
                ...
                Yii::app()->cache->set($cache_key,$data,60*10);
            }
        ...
        public function getSimAffiliateDetails()
        {
            ...
            $data = array();
            $cache_key = "AReportAT_getSimAffiliateDetails"
                . "_start_date_$this->start_date"
                . "_end_date_$this->end_date"
                . "_province_code_$this->province_code"
                . "_status_$this->status";
            ...
            if(!$data){
                $criteria         = new CDbCriteria();
                $criteria->select = "
                    DISTINCT t.order_code, 
                    t.product_name, 
                    t.msisdn, 
                    t.publisher_id, 
                    t.total_money, 
                    t.amount, 
                    t.price as price_sim, 
                    t.vnp_province_id, 
                    t.transaction_id, 
                    t.action_status, 
                    t.type
                ...
                if ($this->start_date && $this->end_date) {
                    $criteria->condition = "
                        t.created_on >= '$this->start_date' 
                        AND t.created_on <= '$this->end_date' 
        ...
        public function getPackageAffiliateDetails()
        {
            ...
            $data = array();
            $cache_key = "AReportAT_getPackageAffiliateDetails"
                . "_start_date_$this->start_date"
                . "_end_date_$this->end_date"
                . "_province_code_$this->province_code"
                . "_status_$this->status";
