<?php
    ...
    class AFTLogs extends FTLogs
    {
        ...
        public function search($order_id)
        {
            ...
            $criteria->compare('object_name', self::OBJECT_ORDER, TRUE);
        ...
        /**
         * @param $order AFTOrders
         * @param $status int
         *
         * @return boolean
         */
        public static function saveLogOrderStatus($order, $status, $old_status = null)
        {
            if(empty($old_status)){
                $old_status = $order->status;
            }
            $log = new AFTLogs();
            $log->object_name       = self::OBJECT_ORDER;
            $log->object_id         = $order->id;
            $log->data_json_before  = CJSON::encode(array('status' => $old_status));
            $log->data_json_after   = CJSON::encode(array('status' => $status));
