...
class APrepaidToPostpaid extends PrepaidToPostpaid
{
	...
	public function search($dataProvider = TRUE)
	{
		...
		if(!$dataProvider){
			return APrepaidToPostpaid::model()->findAll($criteria);
		}
		...
		return new CActiveDataProvider($this, array(
			...
			'pagination' => array(
				'params' => array(
					'get'									=> 1,
					'APrepaidToPostpaid[start_date]'		=> $this->start_date,
					'APrepaidToPostpaid[end_date]'			=> $this->end_date,
					'APrepaidToPostpaid[province_code]'		=> $this->province_code,
					'APrepaidToPostpaid[sale_office_code]'	=> $this->sale_office_code,
				),
				'pageSize' => 50,
			),
		));
...
//		$criteria->compare('note',$this->user_id,note);
	...
	public function getTimeLeftAssign($create_date, $left_hours = 72)
	{
		$time_left = array(
			'time'   => 0,
		...
		if ($create_date) {
			...
			$min      = date('i', $sub_time);
			if ($hour > $left_hours) {
				$time_left['time']   = "Quá hạn (" . ($hour - $left_hours) . "h" . $min . " min)";
				$time_left['status'] = self::EXPIRE;
				$time_left['class_name'] = 'red_color';
			} else {
				if ($hour == 0) {
					$time_left['time'] = ($left_hours - 1) . (60 - $min) . "min";
				} else {
					$time_left['time'] = ($left_hours - $hour) . "h " . (60 - $min) . "min";
				}
