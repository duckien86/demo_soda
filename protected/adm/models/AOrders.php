...
class AOrders extends Orders
{
    ...
    public $phong_bh;
    public $loaihinh_tb;
    public $thangtratruoc;
    public $ckttt;
    public $ckhm;
    public $status_order_fiber;
    ...
    public function rules()
    {
        ...
        return array(
            ...
            array('phone_contact', 'required', 'on' => 'search_fiber', 'message' => Yii::t('web/portal', 'phone_contact_required')),
            ...
            array('last_update, receive_cash_date, channel, affiliate_source, status_shipper, 
                    ...
                    is_pre_order, package_register_date, item_id, item_sim_type, store_id', 'safe'),
            ...
            array('status_fiber,start_date, end_date', 'checkDateRequire', 'on' => 'search, search_fiber'),
            array('end_date', 'checkDate', 'on' => 'admin_complete, search, search_fiber'),
        );
    }
     ...
     *
     * @return CActiveDataProvider | static[] | CDbCriteria
    ...
    public function search($dataProvider = TRUE, $returnCriteria = FALSE)
    {
        ...
        $criteria->select = "
                ...
                (SELECT store_id FROM tbl_sim WHERE order_id = t.id) as 'store_id',
                ...
                (SELECT item_name FROM tbl_order_details WHERE order_id = t.id AND type = 'package') as 'package_name',
                (SELECT price FROM tbl_order_details WHERE order_id = t.id AND type IN ('sim','esim')) as 'total_sim',
        ...
        $criteria->addCondition("t.id IN (SELECT order_id FROM tbl_order_details WHERE type IN ('sim','esim'))");
        ...
        if (!empty($this->status_shipper)) {
            ...
            switch ($this->status_shipper) {
                case 1: // Đã giao, không gửi trả.
                    ...
                    break;
                case 3: // Gửi trả
                    $criteria->addCondition("t.id IN (SELECT os.order_id FROM tbl_order_state os WHERE os.confirm = 3
                            AND os.id = (SELECT max(os2.id) FROM tbl_order_state os2 WHERE os2.order_id = os.order_id))");
                    break;
                case 4: // Hủy
                    $criteria->addCondition("t.id IN (SELECT os.order_id FROM tbl_order_state os WHERE os.confirm = 2
                            AND os.id = (SELECT max(os2.id) FROM tbl_order_state os2 WHERE os2.order_id = os.order_id))");
                    break;
                case 5: // Chưa giao
                    $criteria->addCondition("t.id IN (SELECT order_id FROM tbl_shipper_order WHERE status = 0 AND order_status != 2)");
                    $criteria->addCondition("t.id IN (SELECT order_id FROM tbl_order_state WHERE (confirm NOT IN (2,3))
                ...
                case 6: // Chưa phân công
                    ...
                    $criteria->addCondition("t.id IN (SELECT os.order_id FROM tbl_order_state os 
                            WHERE os.confirm = 10 AND os.id = (SELECT max(os2.id) FROM tbl_order_state os2 
                            WHERE os2.order_id = os.order_id))");
                    $criteria->addCondition("t.id IN (SELECT order_id FROM tbl_sim WHERE order_id != '' AND order_id IS NOT NULL AND status != 10)");
                    break;
            }
        }
        ...
        if (!empty($this->channel)) {
            if ($this->channel == self::CHANNEL_DLTC) {
                ...
                $criteria->addCondition("
                        (t.promo_code IS NOT NULL AND t.promo_code != '')
                        OR (t.affiliate_source = 'freedoo')
                    ");
        ...
        if (!empty($this->sim)) {
            $sim = CFunction::makePhoneNumberBasic($this->sim);
            $sim_84 = CFunction::makePhoneNumberStandard($this->sim);
            $criteria->addCondition("t.id IN (SELECT order_id FROM tbl_sim WHERE order_id IS NOT NULL AND (msisdn LIKE '%$sim%' OR msisdn LIKE '%$sim_84%'))");
        }
        ...
        if (!ADMIN && !SUPER_ADMIN) {
            ...
            $brand_offices_id = (!empty($this->brand_offices_id))
                ...
                : isset(Yii::app()->user->brand_offices_id) ? Yii::app()->user->brand_offices_id : '';
        } else {
            ...
            return new CActiveDataProvider($this, array(
                ...
                'pagination' => array(
                    'params' => array(
                        ...
                        "AOrders[item_sim_type]" => $this->item_sim_type,
    ...
    public function searchfiber($dataProvider = TRUE)
    {
        $criteria = new CDbCriteria();
        $criteria->select = "t.*,
                             t.address_detail,
                             (SELECT name FROM tbl_province WHERE code = t.province_code) as province_code,
                             (SELECT name FROM tbl_district WHERE code = t.district_code) as district_code,
                             (SELECT period FROM tbl_package WHERE code = (SELECT item_id FROM tbl_order_details od1 WHERE order_id = t.id AND od1.type IN ('package_fiber','package_mytv','package_combo','package_home_bundle')))  AS period,
                             (SELECT item_name FROM tbl_order_details WHERE order_id = t.id AND type IN ('package_fiber','package_mytv','package_combo','package_home_bundle')) as package_fiber_name,
                             (SELECT price FROM tbl_order_details WHERE order_id = t.id AND type = 'Datmoi') as cuochoamang,
                             (SELECT price FROM tbl_order_details WHERE order_id = t.id AND type = 'DatCoc') as tienttt,
                             (SELECT price FROM tbl_order_details WHERE order_id = t.id AND type = 'vat') as vat,
                             (SELECT price FROM tbl_order_details WHERE order_id = t.id AND type = 'KMLD') as ckhm,
                             (SELECT price FROM tbl_order_details WHERE order_id = t.id AND type = 'Đặt cọc KM ') as ckttt,
                             (SELECT ngay_ky_hd FROM tbl_reg_fiber rf WHERE rf.freedoo_order_id = t.id) as ngay_ky_hd,
                             (SELECT ten_nv FROM tbl_reg_fiber rf WHERE rf.freedoo_order_id = t.id) as ten_nv,
                             (SELECT ma_nv FROM tbl_reg_fiber rf WHERE rf.freedoo_order_id = t.id) as ma_nv,
                             (SELECT loaihinh_tb FROM tbl_reg_fiber rf WHERE rf.freedoo_order_id = t.id) as loaihinh_tb,
                             (SELECT thangtratruoc FROM tbl_reg_fiber rf WHERE rf.freedoo_order_id = t.id) as thangtratruoc,
                             (SELECT phong_bh FROM tbl_reg_fiber rf WHERE rf.freedoo_order_id = t.id) as phong_bh,
                             (SELECT ma_tb FROM tbl_reg_fiber rf WHERE rf.freedoo_order_id = t.id) as ma_tb,
                             (SELECT stb_use FROM tbl_reg_fiber rf WHERE rf.freedoo_order_id = t.id) as stb_use,
                             (SELECT note FROM tbl_order_state WHERE (id = (SELECT max(os2.id) FROM tbl_order_state os2  WHERE os2.order_id = t.id))) AS note
        ...
        if($this->type_package && $this->type_package != 'all'){
            $criteria->addCondition("t.product_type = '$this->type_package' ");
        }
        if($this->type_package == 'all'){
            $criteria->addCondition("t.product_type IN ('fiber','mytv','combo_fiber_mytv','home_bundle') ");
        }
        if ($this->id) {
            $criteria->addCondition("t.id = '$this->id' ");
        }
        if ($this->full_name) {
            $criteria->addCondition("t.full_name = '$this->full_name' ");
        }
        if ($this->phone_contact) {
            $criteria->addCondition("t.phone_contact = '$this->phone_contact' ");
        }
            ...
            return new CActiveDataProvider($this, array(
                ...
                'pagination' => array(
                    'params' => array(
                        ...
                        "AOrders[end_date]" => $this->end_date,
                        "AOrders[id]" => $this->id,
                        "AOrders[full_name]" => $this->full_name,
                        "AOrders[phone_contact]" => $this->phone_contact,
                        "AOrders[province_code]" => $this->province_code,
                        "AOrders[type_package]" => $this->type_package,
                    ),
                    'pageSize' => 30,
                ),
            ));
    ...
    /*
     * Lấy ra chi tiết đơn hàng fiber
     */

    public function getDetailOrderFiber($dataProvider = TRUE, $returnCriteria = FALSE)
    {
        $start_date = date("Y-m-d", strtotime(str_replace('/', '-', $this->start_date))) . " 00:00:00";
        $end_date = date("Y-m-d", strtotime(str_replace('/', '-', $this->end_date))) . " 23:59:59";
        $criteria = new CDbCriteria();
        $criteria->select = "t.*,
                             ...
                             (SELECT stb_use FROM tbl_reg_fiber rf WHERE rf.freedoo_order_id = t.id) as stb_use,
                             CASE
                                        WHEN ((SELECT confirm FROM tbl_order_state WHERE (id = (SELECT max(os2.id) FROM tbl_order_state os2  WHERE os2.order_id = t.id))))= '10' THEN 'dangthuchien'
                                        WHEN ((SELECT confirm FROM tbl_order_state WHERE (id = (SELECT max(os2.id) FROM tbl_order_state os2  WHERE os2.order_id = t.id))))= '2' THEN 'huy'
                                        WHEN ((SELECT delivered FROM tbl_order_state WHERE (id = (SELECT max(os2.id) FROM tbl_order_state os2  WHERE os2.order_id = t.id))))= '10' THEN 'hoanthanh'
                                    END as status_order_fiber,
                             (SELECT note FROM tbl_order_state WHERE (id = (SELECT max(os2.id) FROM tbl_order_state os2  WHERE os2.order_id = t.id))) AS note
        ...
        if ($this->status_fiber == 2) {
            $criteria->addCondition("(t.id IN 
                    (SELECT os.order_id FROM tbl_order_state os
                        WHERE os.paid = 2 AND os.id = (SELECT max(os2.id) FROM tbl_order_state os2 
                            WHERE os2.order_id = os.order_id
                        )
                    ))");
        }
        if ($this->status_fiber == 3) {
            $criteria->addCondition("(t.id IN 
                    (SELECT os.order_id FROM tbl_order_state os
                        WHERE os.confirm = 2 AND os.id = (SELECT max(os2.id) FROM tbl_order_state os2 
                            WHERE os2.order_id = os.order_id
                        )
                    ))");
        }
        if ($this->status_fiber == 4) {
            $criteria->addCondition("(t.id IN 
                    (SELECT os.order_id FROM tbl_order_state os
                        WHERE os.delivered = 10 AND os.id = (SELECT max(os2.id) FROM tbl_order_state os2 
                            WHERE os2.order_id = os.order_id
                        )
                    ))");
        }
            ...
            return new CActiveDataProvider($this, array(
                ...
                'pagination' => array(
                    'params' => array(
                        ...
                        "AOrders[end_date]" => $this->end_date,
                        "AOrders[province_code]" => $this->province_code,
                        "AOrders[status_fiber]" => $this->status_fiber,
                        "AOrders[type_package]" => $this->type_package,
                    ),
                    'pageSize' => 30,
                ),
    ...
    /*
     * Lấy ra tổng quan đơn hàng fiber
     */
    public function getTotalOrderFiber()
    {
        $data_detail = $this->getDetailOrderFiber(FALSE);
        $arr = array();
        foreach ($data_detail as $item) {
            array_push($arr, $item['status_order_fiber']);
        }
        return $arr;
    }
    ...
    public function search_tool($post = FALSE, $excel = FALSE)
    {
        ...
        $criteria->addCondition("t.agency_contract_id IS NULL OR t.agency_contract_id = ''");
    ...
    public function searchChange($post = FALSE, $excel = FALSE, $pagination = TRUE)
    {
        ...
        $criteria = new CDbCriteria;
        $criteria->select = "t.*,
                (SELECT item_name FROM tbl_order_details WHERE order_id = t.id AND type IN ('sim','esim')) AS 'item_name'
            ";
    ...
    /**
     * Tra cứu đơn hàng rác
     *
     * @param bool $dataProvider
     * @param bool $returnCriteria
     * @return CActiveDataProvider|AOrders[]|CDbCriteria
     */
    public function search_recycle($dataProvider = TRUE, $returnCriteria = FALSE)
    {
        // @todo Please modify the following code to remove attributes that should not be searched.
        ...
        $criteria->addCondition("t.payment_method IS NULL OR t.payment_method = ''");
        $criteria->addCondition("t.id IN (SELECT order_id FROM tbl_order_details WHERE type IN ('sim','esim'))");
        $criteria->addCondition("t.agency_contract_id IS NULL OR t.agency_contract_id = ''");
    ...
    public function search_detail($start_date = '', $end_date = '', $shipper_id = '')
    {
        $criteria = new CDbCriteria;
        $criteria->select = "t.*,
            od.order_id, od.item_id, od.item_name, od.quantity, od.price, od.type,
            so.assign_date as assign_date, so.delivery_date as order_date, so.finish_date as finish_date, sum(od.price) as renueve_order";
    ...
    public function search_complete($post = FALSE)
    {
        ...
        $criteria->select = "t.*,
                od.type AS 'item_sim_type'
            ";

        $criteria->addCondition("t.agency_contract_id IS NULL OR t.agency_contract_id = ''");
        $criteria->addCondition("(os.confirm =10 or os.paid=10) and os.delivered !=10 and t.delivery_type=2");

        if (!empty($this->item_sim_type)) {
            if ($this->item_sim_type == 2) {
                $criteria->addCondition("od.type = 'esim'");
            } else {
                $criteria->addCondition("od.type = 'sim'");
            }
        } else {
            $criteria->addCondition("od.type IN ('sim', 'esim')");
        }
    ...
    public static function getStatusFiber($order_id)
    {
        $status_end = "--";
        $criteria = new CDbCriteria();
        $criteria->condition = " id =(SELECT max(os2.id) FROM tbl_order_state os2  WHERE os2.order_id = '$order_id')";
        ...
        if ($state) {
            if ($state->confirm == 10) {
                $status_end = 'Đang thực hiện';
            } elseif ($state->confirm == 2) {
                $status_end = 'Hủy';
            }
            if ($state->delivered == 10) {
                $status_end = "Hoàn thành";
    ...
    public function getTrafficStatus($order_id)
    {
        ...
        if ($order_id) {
            ...
            $shipper_order = AShipperOrder::model()->find($criteria);
            $order_state = AOrderState::model()->find($criteria);

            if ($shipper_order && $order_state) {
                if ($shipper_order->shipper_id != '' && $shipper_order->order_status != '2') {
                    $status = $shipper_order->status;
                } else if ($shipper_order->order_status == '2' && $order_state->confirm == '2') {
                    $status = ATraffic::CANCEL; // Hủy
                } else if ($shipper_order->order_status == '2' && $order_state->confirm == '3') {
    ...
    public static function getAllTypePackage()
    {
        return array(
            '' => 'Tất cả',
            'fiber' => 'Internet cáp quang',
            'mytv' => 'Truyền hình MyTV',
            'combo_fiber_mytv' =>'Internet & Truyền hình',
            'home_bundle' => 'Internet truyền hình & di động'
        );
    }
    ...
    {
        $province = (!empty($this->province_code)) ? AProvince::getProvinceNameByCode($this->province_code) : "";
        $district = (!empty($this->district_code)) ? ADistrict::getDistrictNameByCode($this->district_code) : "";
        $ward = (!empty($this->ward_code)) ? AWard::getWardNameByCode($this->ward_code) : "";

        if ($this->delivery_type == AOrders::COD) {
    ...
    /**
     * Lấy trạng thái fiber.
     */
    public static function getAllStatusFiber()
    {
        return array(
            1 => "Đang thực hiện",
            3 => "Hủy",
    ...
    public function getTimeLeft($create_date)
    {
        ...
        if ($create_date) {
            ...
            $max_pre_order_execute_date = '2018-10-09 23:59:59';
            $switch_pre_order_date = '2018-09-27 00:00:00';

            $max_execute_hour = 48;
            $now = date('Y-m-d H:i:s');

            if (!empty($this->pre_order_date)) {

                $max_execute_hour = 72;

                if ($this->pre_order_date < $switch_pre_order_date) {
                    if ($now <= $max_pre_order_execute_date) {
                        $sub_time = strtotime($max_pre_order_execute_date) - strtotime($now);
                        $hour = floor($sub_time / (60 * 60));
                        $min = date('i', $sub_time);
                        $time_left['time'] = $hour . "h " . $min . "min";
            ...
            $sub_time = strtotime($now) - strtotime($create_date);
            $days = floor($sub_time / (24 * 60 * 60));
            $hour = floor($sub_time / (60 * 60));
            $min = date('i', $sub_time);

            if ($hour > $max_execute_hour) {
                $time_left['time'] = "Quá hạn (" . ($hour - $max_execute_hour) . "h " . $min . " min)";
                $time_left['status'] = self::EXPIRE;
            } else {
                if ($min > 0) {
                    $max_execute_hour = $max_execute_hour - 1;
                }
                if ($hour == 0) {
                    $time_left['time'] = ($max_execute_hour) . "h " . (60 - $min) . "min";
                    $time_left['status'] = self::NOEXPIRE;
                } else {
                    if ($hour > 36) {
                        $time_left['time'] = ($max_execute_hour - $hour) . "h " . (60 - $min) . "min";
