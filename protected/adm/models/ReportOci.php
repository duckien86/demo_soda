<?php
    ...
    class ReportOci
    {
        ...
        public function getActiveSim(){
            ...
            $sql = "SELECT
                        ID, MSISDN, LOAI_TB, MATINH, 
                        to_char(NGAY_KH,'YYYY-MM-DD - HH24:MI:SS') AS NGAY_KH,
                        to_char(NGAY_HM,'YYYY-MM-DD - HH24:MI:SS') AS NGAY_HM
                        FROM 
                        VSB_ACTIVE
                        ";
            $sql .= " WHERE 1=1";
            if ($this->start_date !== '' && $this->end_date !== '') {
                $sql .= " AND
                                    CREATED_DATE >= to_date ('" . $start_date . "', 'YYYY-MM-DD - HH24:MI:SS')
                                 AND
                                    CREATED_DATE <= to_date ('" . $end_date . "', 'YYYY-MM-DD - HH24:MI:SS')
                                ";
            }
            if ($this->vnp_province_id != '') {
                $sql .= " AND MATINH =:MATINH";
            ...
            // Format dữ liệu từ query DB
            $result = array();
            $data_msisdn = array(
                'MSISDN' => array(),
                'STANDARD_MSISDN' => array(),
            );
            while ($entry = oci_fetch_array($stmt, OCI_ASSOC)) {
                $unique_key = $entry['unique_column'] = $entry['MSISDN'] .'_'. $entry['NGAY_KH'];
                $msisdn = $entry['MSISDN'];
                if(strlen($msisdn) == 9){
                    $entry['MSISDN'] =  '0'.$msisdn;
                    $entry['STANDARD_MSISDN'] =  '84'.$msisdn;
                }else{
                    $entry['MSISDN'] =  CFunction::makePhoneNumberBasic($msisdn);
                    $entry['STANDARD_MSISDN']= CFunction::makePhoneNumberStandard($msisdn);
                }
                if(empty($result[$unique_key])){
                    $data_msisdn['MSISDN'][] = $entry['MSISDN'];
                    $data_msisdn['STANDARD_MSISDN'][] = $entry['STANDARD_MSISDN'];
                    $result[$unique_key] = $entry;
                }
            }
            $result = array_values($result);

            //Lấy mã đơn hàng
            $data_order_id = $this->getOrderID($data_msisdn);

            // gán order_id cho dữ liệu trả về
            $data_return = array();
            foreach($result as $key => $value){
                $msisdn =  $value['MSISDN'];
                if(!empty($data_order_id[$msisdn])){
                    $value['ORDER_ID'] = $data_order_id[$msisdn];
                    $data_return[] = $value;
                }
            }
            return $data_return;
        ...
        public function getOrderID($data_msisdn){
            $order_detail = $this->getOrderDetailBYListItemName($data_msisdn['MSISDN']);
            $standard_order_detail = $this->getOrderDetailBYListItemName($data_msisdn['STANDARD_MSISDN']);

            $result = array();
            foreach($order_detail as $key => $value){
                if(!empty($value->item_name)){
                    $msisdn_key = CFunction::makePhoneNumberBasic($value->item_name);
                    $result[$msisdn_key] = $value->order_id;
                }
            }
            foreach($standard_order_detail as $key => $value){
                if(!empty($value->item_name)){
                    $msisdn_key = CFunction::makePhoneNumberBasic($value->item_name);
                    $result[$msisdn_key] = $value->order_id;
                }
        ...
        public function getOrderDetailBYListItemName($msisdn){
            $criteria = new CDbCriteria();
            $criteria->select = 'order_id, item_name';
            $criteria->addInCondition('type', array('sim','esim'));
            $criteria->addInCondition('item_name',$msisdn);
            return $order_detail = AOrderDetails::model()->findAll($criteria);
        }
