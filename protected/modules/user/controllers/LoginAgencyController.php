<?php
    ...
    class LoginAgencyController extends Controller
    {
        ...
        {
            if(!strpos(Yii::app()->request->requestUri, 'agency')){
                    ...
                    if ($model->validate()) {
                        ...
                        if($user){
                            if(!empty($user->agency_id)){
                                Yii::app()->user->setState('agency', $user->agency_id);
                                ...
                                if(!empty($user->province_code)){
                                    $province = AProvince::model()->findByAttributes(array('code' => $user->province_code));
                                    if ($province) {
        ...
        public function actionResendOtp()
        {
            if(isset($_GET['data'])){
                ...
                if(!$model->validate()){
                    return $this->redirect('index.php?r=user/loginAgency');
                }
                ...
                return $this->redirect('index.php?r=user/loginAgency/otp&data=' . $data);
            }
            return $this->redirect('index.php?r=user/loginAgency');
        }
