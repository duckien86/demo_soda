    ...
    <div class="item_package item_package_fiber">
                    ...
                    <div class="">
                        <div class="new-price" style="float: left; margin-right: 5px; width: 100%; font-size: 20px">
                            <?php echo number_format($model->price_discount) ?><sup>đ</sup><span><?php echo '/' . WPackage::model()->getPackagePeriodLabel($model->period) ?></span>
                             <span style="text-decoration: none !important; color: #fff; padding: 2px 10px; background: red; border-radius: 5px; margin-left: 12%">
                          <?php if($model->price_discount){ ?>
        ...
        <div class="action text-center cus-btn">
            <a class="btn btn-detail" data-toggle="modal" data-target="#popup_<?php echo $model->id ?>">Chi
                tiết</a>
            <?php if ($model->freedoo == 0) { ?>
                <a href="<?php echo Yii::app()->controller->createUrl('package/registercomboallprovince', array('package' => $model->id)); ?>"
                   class="btn btn-register" onclick="nonepopup();">
                    <?php echo CHtml::encode(Yii::t('web/portal', 'register')); ?>
                </a>
            <?php } elseif ($model->freedoo == 1 || $model->freedoo == 2) { ?>
                <a href="<?php echo Yii::app()->controller->createUrl('package/registercombo', array('package' => $model->id)); ?>"
                   class="btn btn-register" onclick="nonepopup();">
                    Đăng ký
                </a>
            <?php } ?>
