...
<div class="card">
                    ...
                    <li>
                        <div class="radio radio-btn-bg">
                            ...
                            <label class="touch" for="WOrderDetails_price_<?= $key ?>"
                                   style="background: url('<?= Yii::app()->theme->baseUrl ?>/images/<?= $key ?>.png') no-repeat;background-size: 100% 100%">
                            </label>
    ...
    <div class="form-group">
        <?php echo CHtml::label(Yii::t('web/portal', 'input_email') . ' <span class="required">*</span>', 'WOrders_email', array('class' => 'label_text')) ?>
        <?php echo $form->textField($modelOrder, 'email',
            array('class'    => 'textbox_lg', 'maxlength' => 255,
                  'onchange' => 'getOrderPrice();'
            )); ?>
        <?php echo $form->error($modelOrder, 'email'); ?>
        <div class="space_10"></div>
        <div class="note">
            Chúng tôi sẽ gửi mã thẻ đến email này
        </div>
    </div>
    <div class="form-group">
        <?php
            echo $form->labelEx($orderDetails, 'quantity', array('class' => 'label_text'));
            echo $form->numberField($orderDetails, 'quantity', array(
                'class' => 'textbox_lg', 'maxlength' => 255, 'min' => 1, 'onchange' => 'getOrderPrice();'
            ));
...
<script>
    ...
    function getOrderPrice() {
        var form_data = new FormData(document.getElementById("card_form"));//formID
        $.ajax({
            type: "POST",
